# CODE COPIED FROM https://arxiv.org/pdf/2012.03801.pdf
import sys
import torch
import numpy as np
...
class FullHessian:
    ...
    def Hv(self, v):
        ...
        for iter, batch in enumerate(self.loader):
            ...
            input, target, lengths = batch[0], batch[1], batch[2]
            input, target, lengths = input.type(dtype).to(self.device), target.type(dtype).to(self.device), lengths.to(self.device)
    
            f = self.model(input, lengths)
            ...
            if self.hessian_type == 'G':
                ...
                # (d f / d theta) v
                print(zT_df_dtheta.dtype, z.dtype, v.dtype)
    ...
    def Lanczos(self, M):
        print("lanczos")
        v = self.my_randn() # random parameters
        v = self.my_div_const(v, self.my_norm(v)) # normalize them with their l2 norm
        ...
        if self.double:
            ...
            bet = bet.double()
        dtype = torch.cuda.FloatTensor if torch.cuda.is_available() else torch.FloatTensor
        v = [v[i].type(dtype).to(self.device) for i in range(len(v))]
        
        alp = alp.type(dtype).to(self.device)
        bet = bet.type(dtype).to(self.device)
...
class LayerHessian:
    ...
    # M iteratinos of Lanczos on the linear operator mat_vec
    def Lanczos(self, M):
        v = self.my_randn()  # take random parameters
        v = self.my_div_const(v, self.my_norm(v)) #normalize them by their l2 norm
        ...
        for j in range(M):
            ...
            sys.stdout.flush()
            v_next = self.mat_vec(v) # multiply vector with the hessian 
